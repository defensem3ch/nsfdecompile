...
OCC_Decompiler* Create_OCCDecompiler()
{
	OCC_Decompiler* Dec;
	Dec = calloc(1, sizeof(OCC_Decompiler));
	Dec->dpcm_inst = -1;
	return Dec;
}

int OCC_LoadNSF(OCC_Decompiler* Dec, char* filename, int version)
{
	NSF_Header h;
	NSF_Song* Song;
	NSF_Pattern* Pattern;
	...
	int* pp_pattern;
	int src_size;
	int pp_data, pp_song, pp_inst, pp_entry, pp_sample, pp_groove, pp_fds, flags;
	int pp_dpcm;
	int div_ntsc;
	int offset;
	int channels;
	...
	//printf("LOAD: 0x%04X INIT: 0x%04X PLAY: 0x%04X\n", h.load_addr, h.init_addr, h.play_addr);
	//printf("Expansion: 0x%02X channels: %i\n", h.expansion, channels);
	...
	pp_groove = pp_data + ptr[8] + 256*ptr[9];
	flags = ptr[10];
	pp_fds = pp_data + ptr[11] + 256*ptr[12];
	pp_dpcm = 0x80 + 0xC000 - h.load_addr;

	ptr += 9;
	ptr += (h.expansion & 0x04)? 2: 0;
	ptr += (Dec->version >= OCC_3_4)? 2: 0;
	...
	if(Dec->version < OCC_3_4) pp_groove = pp_song;

	jmax = pp_song - pp_groove;
	ptr = buf + pp_groove + 1;
	Groove = Dec->Groove;
	k = 0;
	...
	for(j=1;j<jmax;j++){
		byte_in = *(ptr++);
		if(byte_in){
			Groove->data[k++] = byte_in;
			Groove->length = k;
		} else {
			Groove->index = Groove - Dec->Groove;
			k = 0;
			Groove->offset = *(ptr++);
			Dec->n_grooves++;
			Groove++;
	...
	Dec->n_entries = (pp_sample - pp_entry) / 3;
	Dec->n_samples = (pp_groove - pp_sample) / 3;
...
int OCC_DecompilePattern(OCC_Decompiler* Dec, NSF_Pattern* Pattern, unsigned char* ptr)
{
	NSF_Item Item = {0};
	NSF_Item Empty = {0};
	//unsigned char* buf;
	...
	int fixed = 0;
	int duration = 0;
	int setd_byte = (Dec->version >= OCC_3_8)? 0x82: 0xB0;
	int resd_byte = (Dec->version >= OCC_3_8)? 0x84: 0xB2;
	int i;
	...
	while(row < Pattern->rows){
		...
		case 0x8: case 0x9: case 0xA: case 0xB: case 0xC: case 0xD:
			...
			if(byte_in > CMD_EFF_NOTE_RELEASE){
				switch(Dec->version){
				case OCC_3_0:
				case OCC_3_1:
					byte_in += 6;
				...
				case OCC_3_3:
					if(byte_in > CMD_EFF_LINEAR_COUNTER) byte_in += 4;
					break;

				case OCC_3_4:
					if(byte_in > CMD_EFF_GROOVE) byte_in += 2;
					break;
			...
			t_fxtype = Dec->version >= OCC_3_8? convert_new_0cc_cmd(byte_in): convert_0cc_cmd(byte_in);
			...
			default:
				...
				case EF_GROOVE:
					for(i=0;i<Dec->n_grooves;i++){
						if(t_fxvalue != Dec->Groove[i].offset) continue;
						t_fxvalue = i;
						break;
					}
					break;
...
int OCC_ExportFTM(OCC_Decompiler* Dec, char* filename)
{
	FT_File* File;
	FT_Track* Track;
	NSF_Song* Song;
	...
	for(i=0;i<Dec->n_entries;i++){
		mult = (Dec->version < OCC_3_8)? 3: 1;
		sub = (Dec->version < OCC_3_8)? 0: 1;
	...
	memcpy(File->Groove, Dec->Groove, 32 * sizeof(NSF_Groove));
	File->n_grooves = Dec->n_grooves;
